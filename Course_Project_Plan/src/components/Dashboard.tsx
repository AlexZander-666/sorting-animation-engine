...
import {
  ...
  generateDuplicateHeavyArray,
  generateOutlierArray,
  sanitizeInput,
} from '../utils/data';
...
const Dashboard = () => {
  ...
  const handlePreset = (
    preset: 'nearlySorted' | 'reverse' | 'duplicates' | 'outliers',
  ) => {
    let values: number[] = [];
    switch (preset) {
      case 'nearlySorted':
        values = generateNearlySortedArray(selectedSize);
        break;
      case 'reverse':
        values = generateReverseArray(selectedSize);
        break;
      case 'duplicates':
        values = generateDuplicateHeavyArray(selectedSize);
        break;
      case 'outliers':
        values = generateOutlierArray(selectedSize);
        break;
      default:
        values = generateRandomArray(selectedSize);
    }
    setError(undefined);
    setData(values);
    setInputValue(values.join(', '));
      ...
      <div className="dashboard-grid">
        <div className="panel">
          ...
          <div className="preset-controls">
            ...
            <button
              type="button"
              onClick={() => handlePreset('nearlySorted')}
            >
              近乎有序
            </button>
            <button type="button" onClick={() => handlePreset('reverse')}>
              逆序
            </button>
            <button
              type="button"
              onClick={() => handlePreset('duplicates')}
            >
              重复值多
            </button>
            <button
              type="button"
              onClick={() => handlePreset('outliers')}
            >
              少量离群值
            </button>
          </div>
          <p className="note-text">
            说明：数值会被限制在 {MIN_VALUE}-{MAX_VALUE}，以保证柱状图可读。
            若需 0 或负数，请在 Benchmark 模式下使用自定义输入。
          </p>
        </div>
        <div className="panel">
          <h3>算法与播放控制</h3>
