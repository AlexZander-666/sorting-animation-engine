...
2. **文件导入**：点击 “导入 CSV/TXT”，上传包含逗号或换行分隔数字的纯文本文件。
   - 文件大小限制为 10MB；
   - 如果格式不正确，系统会显示错误提示。
3. **随机生成**：选择预设大小（10 / 30 / 50 / 100 / 200 / 500），点击“随机生成”；常用分布包括近乎有序、逆序、重复值多、少量离群值。

> **提示**：可视化模式最多显示 512 个元素，超出部分会直接阻断动画并提示切换 Benchmark。输入会自动过滤非数字，并将数值限制在 5-100 范围内（可视化保证可见度，0/负数请在 Benchmark 模式查看原值）。

### 4.3 算法选择与播放
1. 在 “算法与播放控制” 面板中勾选目标算法；
2. 点击 “开始” 触发动画播放，使用 “暂停” / “重置” 控制播放状态；
3. 通过 “速度 (ms)” 滑块调节动画节奏；当元素较多时，柱状图会自动降采样显示（保留首尾与等间距索引）。
